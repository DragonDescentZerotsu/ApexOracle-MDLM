...
if __name__ == '__main__':
    ...
    model_save_dir = current_directory / 'Checkpoints' / 'genome_text_learnable_emb' / 'guidance_regressor_non_pad_noise'
    ...
    for i, (strain_for_train, test_group_name) in enumerate(zip(train_groups, group_names)):
        ...
        for ensemble in tqdm(range(num_ensembles), desc=' Doing ensembles '):
            ...
            DIT_ckpt_path = '/data2/tianang/projects/mdlm/Checkpoints_fangping/1-255000-fine-tune.ckpt'  # /data2/tianang/projects/mdlm/Checkpoints_fangping/last_reg_v1.ckpt
            ...
            if resume_from_ckpt:
                logger.info(f' Resume training from epoch: {resum_epoch}')
                ckpt_path = f'/data2/tianang/projects/Synergy/Checkpoints/genome_text_learnable_emb/guidance_regressor_non_pad/noise_guidance_all_peptide_epoch_{resum_epoch}_of_200.pth'
                ckpt = torch.load(ckpt_path, map_location=device)
                co_cross_attn_genome.load_state_dict(ckpt['co_cross_attn_genome'])
                co_cross_attn_text.load_state_dict(ckpt['co_cross_attn_text'])
                reg_head.load_state_dict(ckpt['re_head_state_dict'])
                cls_head.load_state_dict(ckpt['cls_head_state_dict'])
                learnable_embedding_weight.data = ckpt['learnable_embedding_weight']
                mdlm_model.load_state_dict(ckpt['mdlm_model_state_dict'])
    ...
    # Test R2: {r2:.6f}, genome text Test R2: {gt_r2:.6f}, text only Test R2: {t_r2:.6f}, Test train mean MIC R2: {r2_train_mean:.6f}""")
                ...
                for gt_batch, t_batch in tqdm(
                    ...
                    if gt_batch is not None:
                        input_ids = gt_batch['input_ids'].to(device)
                        attention_mask = gt_batch['attention_mask'].to(torch.bool).to(device)
                        # attention_mask = torch.ones_like(gt_batch['attention_mask']).to(torch.bool).to(device)
                        labels = gt_batch['label'].to(device)
                        padded_genome_embeddings = gt_batch['padded_genome_embeddings']  # .to(torch.float)
                        genome_attn_masks = gt_batch['genome_attn_masks']
                        padded_text_embeddings = gt_batch['padded_text_embeddings']  # .to(torch.float)
                    ...
                    if t_batch is not None:
                        input_ids = t_batch['input_ids'].to(device)
                        attention_mask = t_batch['attention_mask'].to(torch.bool).to(device)
                        labels = t_batch['label'].to(device)
