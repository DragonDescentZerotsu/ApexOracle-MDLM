...
if __name__ == '__main__':
    ...
    pooling_method = 'mean_wo_pad_eval' # cls_wo_pad / mean_w_pad / cls_w_pad / mean_wo_pad / cls_wo_pad_eval / mean_wo_pad_eval
    ...
    Pep_dict = {}
    for Pep_id, Pep_input_id in tqdm(zip(Pep_ids, Pep_input_ids), desc='filtering peptides'):
        if Pep_id not in Pep_dict.keys():
            # input_ids_list = ast.literal_eval(Pep_input_id)
            # if len(input_ids_list) <= 512:
            Pep_dict[Pep_id] = torch.from_numpy(np.array(ast.literal_eval(Pep_input_id)))
    ...
    Pep_emb ={}
    for Pep_id, Pep_input_id in tqdm(Pep_dict.items(), desc='computing peptide embeddings:'):
        Pep_input_id = Pep_input_id.to(device)
        with torch.amp.autocast('cuda', enabled=True):
            if pooling_method == 'cls_wo_pad':
                Pep_emb[Pep_id] = mdlm_model(Pep_input_id.unsqueeze(0))[:, 0, :].detach().cpu()
            if pooling_method == 'cls_wo_pad_eval':
                ...
                Pep_emb[Pep_id] = torch.mean(output, dim=0)
            elif pooling_method == 'cls_w_pad':
                Pep_input_id = Pep_input_id.unsqueeze(0)
                seq_len = Pep_input_id.shape[-1]
                paddings = torch.ones([1, 1024]) * tokenizer.pad_token_id
                paddings[0, :seq_len] = Pep_input_id[0, :seq_len]
                Pep_emb[Pep_id] = mdlm_model(paddings.long().to(device))[:, 0, :].detach().cpu()
    ...
    torch.save(Pep_emb, current_directory / 'DataPrepare' / 'Data' / f'Pep_emb_dict_{pooling_method}.pt')
