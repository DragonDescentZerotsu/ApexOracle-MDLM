...
class Pep_SM_Classifier(L.LightningModule):
    def __init__(self, config, vocab_size, backbone_ckpt_path, mask_index, lr, num_epochs, pos_weight, pad_token_id):
        super().__init__()
    ...
    def training_step(self, batch, batch_idx):
        ...
        attention_mask = batch['input_ids'] != self.hparams.pad_token_id
        max_seq_len = attention_mask.sum(dim=1).max()
        input_ids = batch['input_ids'][:, :max_seq_len]
        attention_mask = attention_mask[:, :max_seq_len].to(torch.bool)
...
if __name__ == '__main__':
    ckpt_cb = ModelCheckpoint(
        dirpath=str(current_directory / "Checkpoints" / "lg_outputs"/ "checkpoints"),
    ...
    wandb_logger = WandbLogger(
        ...
        name="run1",  # 本次运行的显示名称
        save_dir=str(current_directory / "Checkpoints" / "lg_outputs" / "wandb_logs"),  # 本地日志保存路径
    ...
    trainer.fit(model, train_loader, val_loader,
                ckpt_path='/data2/tianang/projects/mdlm/checkpoints/last.ckpt')
