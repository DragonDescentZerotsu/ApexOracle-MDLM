...
def load_torch_file(
    ...
    *,
    ...
    try:
        ...
        # Torch versions before ``weights_only`` was added do not accept the keyword.
...
def extract_state_dict(
    ...
    *,
    ...
    if key not in payload:
        ...
        raise KeyError(f"Checkpoint does not contain {key!r}; available keys: {available or '<none>'}.")

    state_dict = payload[key]
    if not isinstance(state_dict, Mapping):
        raise TypeError(f"Checkpoint field {key!r} must be a mapping, got {type(state_dict).__name__}.")
    return state_dict
