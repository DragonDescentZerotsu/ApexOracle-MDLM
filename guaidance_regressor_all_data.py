...
class FirstTokenAttention_genome(nn.Module):
    ...
    def forward(self, mol_cls_emb, genome_embs, key_padding_mask, **kwargs):
        ...
        attn_output, attn_weights = self.mha(query_norm, key_value[:, :, :genome_embs_dim], key_value[:, :, genome_embs_dim:], key_padding_mask = key_padding_mask.to(torch.bool), average_attn_weights=True)  # (1, batch_size, embed_dim)
